...
CREATE VIEW quantN AS
SELECT cust, MAX(quant) AS Max_Q, MIN(quant) AS Min_Q, round(AVG(quant),0) AS Avg_Q
FROM sales
GROUP BY cust;

CREATE VIEW maxmum AS
SELECT q.cust, q.Max_Q, s.prod,s.month,s.day,s.year, s.state
FROM quantN AS q, sales AS s	
WHERE q.cust = s.cust AND q.Max_Q = s.quant;

CREATE VIEW minimum AS
SELECT q.cust, q.Min_Q, s.prod,s.month,s.day,s.year, s.state
FROM quantN AS q, sales AS s	
WHERE q.cust = s.cust AND q.Min_Q = s.quant;

SELECT x.cust, x.Max_Q, x.prod, x.month, x.day, x.year, x.state,y.Min_Q, y.prod, y.month, y.day, y.year, y.state, q.Avg_Q
FROM maxmum AS x, minimum AS y, quantN AS q
WHERE x.cust = y.cust AND x.cust = q.cust;
